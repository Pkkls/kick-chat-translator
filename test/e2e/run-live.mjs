 ...
 *
 *   node test/e2e/run-live.mjs               the anonymous ones
 *   node test/e2e/run-live.mjs --avec-session   those too
 *   node test/e2e/run-live.mjs --only lang-menu-live,bar-panel-mesure
 *   node test/e2e/run-live.mjs --jobs 1
 *
 ...
 * test/e2e/profile-live, which only exists on a machine where someone
...
function lancer(nom) {
  return new Promise((resolve) => {
    ...
    const p = spawn('node', [`test/e2e/${nom}.mjs`], { cwd: ROOT, encoding: 'utf8' });
    let sortie = '';
