...
## [2.6.0](https://github.com/Pkkls/kick-chat-translator/releases/latest) の新着

ラテン文字で書かれたチャットがふたたび翻訳されるようになりました。読む言語を英語にしていると、
基本ラテン文字が85%を超えるテキストはすでに英語であるかのように弾かれ、スペイン語・トルコ語・
フランス語・ポルトガル語が対象外になる一方で、日本語と韓国語はそのまま通っていました。保存した
チャットで計測した結果、スペイン語は76行中66行、トルコ語は51行中40行が落とされ、韓国語は43行中
0行でした。

チャット上部のバーは消えなくなり、インターフェースは10言語に対応し、用語集がついに編集できるよう
になり、デバッグタブでその行が翻訳されなかった理由を確認できます。全一覧は
...
Chrome/Edge ではハードウェアが対応していれば端末内翻訳も使えます（通信なし・上限なし）。Brave と
Firefox はまだこの API を搭載していないので、クラウドのチェーンにフォールバックします。
