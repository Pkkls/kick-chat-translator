...
## [2.7.0](https://github.com/Pkkls/kick-chat-translator/releases/latest) の新着

**読む言語を変えると、すでに画面にある行も切り替わるようになりました。** これまでは後から届いた
メッセージにしか効かず、すでに表示されていたものはページを再読み込みするまで前の言語のままでした。
表示スタイルとバッジも同じでした。

**引き伸ばされたメッセージに二度目の機会を。** チャットには `muuuuy biennnn` や `BINNNNNGOOOOO` と
書かれ、翻訳サービスはそれを未翻訳のまま返してきます。そうなった行は、圧縮した文字列で一度だけ
再試行するようになり、`muy bien` と `BINGO` が返ってきます。拒否された後だけで、先にではありません。
サービスは一定の引き伸ばしなら扱えるので、最初からすべて圧縮すると、そうした場合はかえって悪化
します。

**DeepL のキーは、入力した端末にとどまります。** これまではアカウントにログイン中のすべての Chrome
に同期されていました。既存のキーはひとりでに移動します。

このほか、入力プレビューは確信がないかぎり書いた言語をエンジンに伝えなくなり、ブラウザのワーカー
起動が遅かったときにページ上で黙り込むことがなくなり、Kick の各ページが読み込むスクリプトは15%
軽くなりました。全一覧は [CHANGELOG.md](CHANGELOG.md) にあります。
