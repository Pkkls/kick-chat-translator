...
## [2.8.0](https://github.com/Pkkls/kick-chat-translator/releases/latest) の新機能

**Kick のメッセージ入力欄に言語ボタンを追加。** 書く言語を切り替えるには、これまでチャット上部のバーまで往復する必要がありました。ボタンはチャットの操作ボタンの並びにあるので、ポインタが入力欄から離れません。クリックでチャンネルの言語と直前に選んだ言語を切り替え、矢印をクリックすると全一覧が開きます。最初に選んだ言語がそのままお気に入りになり、設定を書き込む必要はありません。

**デスクトップがライト、Kick がダークという環境では、翻訳が見えていませんでした。** 挿入したテキストが、置かれているチャットではなく OS の設定に従っていたため、Kick の暗い背景に暗い文字を描いていました。実測 1.01:1、つまりコントラストがまったくない状態です。現在はチャット自身の背景を読み取り、Kick のテーマ切り替えにも再読み込みなしで追従します。

**「置き換え」が実際に置き換えるようになりました。** どの操作からも選べない 4 番目の設定値で、名前に反して「インライン」と同じ表示のまま原文も横に残していました。インラインが 9 件のところ 12 件表示します。「ホバー時」も、実際にホバーしたかどうかに関わらず全メッセージの下にラベルを書くのをやめました。これがチャットの表示件数を 3 分の 1 削っていました。

**チャット、バー、すべての言語メニューが、ブラウザの言語ではなく選んだ言語で表示されます。** 10 言語すべてで対応しました。

そのほか: アラビア語で正しく左右反転するようになり、長い URL やスパムの連続がチャットを横に押し出さなくなり、失敗した行が 1 行を占有しなくなり、再試行ボタンがキーボードとタッチで操作でき、動きを止めるよう OS に設定している人にはすべてのアニメーションが止まり、インターフェース全体がキーボードで辿れてすべての操作要素に名前が付きました。全一覧は [CHANGELOG.md](CHANGELOG.md) にあります。
---
...
- **表示**：訳文をメッセージの下・同じ行・そのあとに小さめの斜体・ホバー時のいずれかで表示。原文、翻訳元の言語、プロバイダーのバッジはそれぞれ任意。設定画面のサンプル行で、選ぶ前に各スタイルを確認できます
- **言語ボタン**: Kick のメッセージ入力欄、エモートアイコンの隣にあるチップ。クリックでチャンネルの言語と直前に選んだ言語を切り替え、長押しで一覧を開き、2文字打てば絞り込めます。書く言語を変えるのにチャット上部まで戻らずに済むよう、この位置にあります
