...
describe('le detecteur sur du chat qu il n a jamais vu', () => {
  ...
  it('rappelle un tiers de moins en brut, mais la longueur en explique la moitie', () => {
    expect(plain(SURE2.total)).toEqual({ right: 121, silent: 139, wrong: 0 });
    expect(rappel(SURE1.total)).toBe(55);
    expect(rappel(SURE2.total)).toBe(47);
  ...
  it('memorise trois fois moins au-dela de la borne du lexique', () => {
    ...
    expect(rappel(runMatrix(confidentLanguage, bande(LANG_CHAT, long)).total)).toBe(53);
    expect(rappel(runMatrix(confidentLanguage, bande(LANG_CHAT2, long)).total)).toBe(44);
  });
  ...
  it('montre que seul le chemin qui depend du lexique perd au changement de corpus', () => {
    expect(plain(BRUT2.total)).toEqual({ right: 167, silent: 36, wrong: 57 });
