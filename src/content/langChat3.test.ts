...
describe('le corpus de reglage', () => {
  ...
  it('en nomme 100 sur 260', () => {
    expect(plain(SURE3.total)).toEqual({ right: 100, silent: 160, wrong: 0 });
    expect(plain(BRUT3.total)).toEqual({ right: 155, silent: 39, wrong: 66 });
  });
  ...
  it('ne bouge que par un mecanisme qui transfere, pas par le lexique', () => {
    expect(SURE3.total.right).toBe(100);
  });
