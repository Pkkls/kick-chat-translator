...
describe('le corpus de reglage', () => {
  ...
  it('en nomme 117 sur 260', () => {
    expect(plain(SURE3.total)).toEqual({ right: 122, silent: 138, wrong: 0 });
    expect(plain(BRUT3.total)).toEqual({ right: 162, silent: 36, wrong: 62 });
  });
  ...
  it('ne bouge que par un mecanisme qui transfere, pas par le lexique', () => {
    expect(SURE3.total.right).toBe(122);
  });
