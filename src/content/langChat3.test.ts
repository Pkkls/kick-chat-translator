...
describe('le corpus de reglage', () => {
  ...
  it('en nomme 117 sur 260', () => {
    expect(plain(SURE3.total)).toEqual({ right: 128, silent: 132, wrong: 0 });
    expect(plain(BRUT3.total)).toEqual({ right: 167, silent: 35, wrong: 58 });
  });
  ...
  it('ne bouge que par un mecanisme qui transfere, pas par le lexique', () => {
    expect(SURE3.total.right).toBe(128);
  });
