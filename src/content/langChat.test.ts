...
describe('le chemin sur, sur du chat', () => {
  ...
  it('se tait sur trois lignes de chat sur quatre', () => {
    expect(plain(CHAT_SURE.total)).toEqual({ right: 123, silent: 267, wrong: 0 });
    expect(muet(CHAT_SURE.total)).toBe(68);
  });
  ...
  it('perd un tiers de son rappel quand les diacritiques tombent', () => {
    expect(plain(NU_SURE.total)).toEqual({ right: 78, silent: 312, wrong: 0 });
    expect(muet(NU_SURE.total)).toBe(80);
  });
});
...
describe('le chemin brut, sur du chat', () => {
  ...
  it('se trompe sur plus d une ligne de chat sur quatre', () => {
    expect(plain(CHAT_BRUT.total)).toEqual({ right: 199, silent: 87, wrong: 104 });
  ...
  it('empire encore sans diacritiques', () => {
    expect(plain(NU_BRUT.total)).toEqual({ right: 164, silent: 112, wrong: 114 });
  });
