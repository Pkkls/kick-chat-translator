...
describe('le chemin sur, sur du chat', () => {
  ...
  it('se tait sur trois lignes de chat sur quatre', () => {
    expect(plain(CHAT_SURE.total)).toEqual({ right: 222, silent: 168, wrong: 0 });
    expect(muet(CHAT_SURE.total)).toBe(43);
  });
  ...
  it('perd un tiers de son rappel quand les diacritiques tombent', () => {
    expect(plain(NU_SURE.total)).toEqual({ right: 147, silent: 243, wrong: 0 });
...
describe('le chemin brut, sur du chat', () => {
  ...
  it('se trompe sur plus d une ligne de chat sur quatre', () => {
    expect(plain(CHAT_BRUT.total)).toEqual({ right: 263, silent: 57, wrong: 70 });
  ...
  it('empire encore sans diacritiques', () => {
    expect(plain(NU_BRUT.total)).toEqual({ right: 207, silent: 91, wrong: 92 });
  });
