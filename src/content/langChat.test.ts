...
describe('le chemin sur, sur du chat', () => {
  ...
  it('se tait sur trois lignes de chat sur quatre', () => {
    expect(plain(CHAT_SURE.total)).toEqual({ right: 225, silent: 165, wrong: 0 });
    expect(muet(CHAT_SURE.total)).toBe(42);
  });
  ...
  it('perd un tiers de son rappel quand les diacritiques tombent', () => {
    expect(plain(NU_SURE.total)).toEqual({ right: 150, silent: 240, wrong: 0 });
    expect(muet(NU_SURE.total)).toBe(62);
  });
});
...
describe('le chemin brut, sur du chat', () => {
  ...
  it('se trompe sur plus d une ligne de chat sur quatre', () => {
    expect(plain(CHAT_BRUT.total)).toEqual({ right: 266, silent: 54, wrong: 70 });
  ...
  it('empire encore sans diacritiques', () => {
    expect(plain(NU_BRUT.total)).toEqual({ right: 210, silent: 88, wrong: 92 });
  });
