...
describe('baseline, 2026-09-21, Tatoeba corpus', () => {
  ...
  it('confidentLanguage stays quiet and is rarely wrong', () => {
    expect(plain(CONFIDENT.total)).toEqual({ right: 3325, silent: 1708, wrong: 7 });
  });

  it('confidentLanguage on short lines, the regime a chat lives in', () => {
    expect(plain(CONFIDENT.shortOnly)).toEqual({ right: 961, silent: 712, wrong: 7 });
  });
  ...
  it('detectLanguage is wrong on a fifth of all lines', () => {
    expect(plain(DETECT.total)).toEqual({ right: 3890, silent: 483, wrong: 667 });
  });

  it('detectLanguage is wrong on a quarter of short lines', () => {
    expect(plain(DETECT.shortOnly)).toEqual({ right: 1125, silent: 271, wrong: 284 });
  });
});
