...
export const ru: Record<string, string> = {
  ...
  'Kick Chat Translator reads chat as it arrives and puts a translation under each message in the language you read. It also translates what you type before you send it. No account, no tracking, open source.': 'Kick Chat Translator читает чат по мере поступления и ставит под каждым сообщением перевод на ваш язык. Он также переводит то, что вы пишете, до отправки. Без аккаунта, без слежки, с открытым кодом.',
  'Version': 'Версия',
  'Languages': 'Языки',
  'Providers ready': 'Готовых поставщиков',
  'Messages translated': 'Переведено сообщений',
  'How a message becomes a translation': 'Как сообщение становится переводом',
  'The message is read from the page, emotes and links set aside.': 'Сообщение читается со страницы, эмоции и ссылки откладываются.',
  'Its language is worked out on the spot, from the script and the words it uses. This answer decides everything after it.': 'Язык определяется на месте, по письменности и словам. Этот ответ решает всё остальное.',
  'Filters run: already in your language, too short, a bot, a language you did not pick.': 'Работают фильтры: уже на вашем языке, слишком коротко, бот, невыбранный язык.',
  'The cache is asked first, so the same message is never paid for twice.': 'Сначала спрашивается кэш, чтобы не платить дважды за одно и то же.',
  'Otherwise the first available provider in your chain answers, or the on-device engine if you enabled it.': 'Иначе отвечает первый доступный поставщик цепочки или встроенный движок.',
  'The Activity tab shows what each of these steps did for you.': 'Вкладка «Активность» показывает, что сделал каждый шаг.',
  'What changed in this version': 'Что изменилось в этой версии',
  'entries': 'записей',
  'of the cap': 'от предела',
  'oldest kept in memory:': 'самая старая в памяти:',
  'requests have gone through this limit': 'запросов прошло через этот предел',
  'never reached so far': 'ни разу не достигнут',
  'messages held back by this cap': 'сообщений задержано этим пределом',
  'Maintenance': 'Обслуживание',
  'Nothing picked: every language is translated.': 'Ничего не выбрано: переводятся все языки.',
  'picked: everything else is left alone.': 'выбрано: всё остальное остаётся как есть.',
  'channels you have watched': 'каналы, которые вы смотрели',
  'Only here': 'Только здесь',
  'Never here': 'Здесь никогда',
  'No key yet, so DeepL is skipped and the chain moves on.': 'Ключа пока нет, поэтому DeepL пропускается.',
  'used this month': 'использовано в этом месяце',
  'messages translated': 'сообщений переведено',
  'served from cache': 'из кэша',
